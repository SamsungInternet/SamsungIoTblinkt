...
var intervalObjOnOff = setInterval(setRainbow, 1000, pixels, brightness);


function stopRainbowLight(arg) {
  console.log(`Rainbw Light Stopped => ${arg}`);
  clearInterval(intervalObjOnOff);
  leds.setup();
  leds.clearAll();
  leds.sendUpdate();
...
setTimeout(stopRainbowLight, 20000, 'setTimeout called');
...
module.exports = {
  ...
  startRainbowLight: function() {
    console.log('** Starting Rainbow Light **');
    intervalObjOnOff = setInterval(setRainbow, 1000, pixels, brightness);
    setTimeout(stopRainbowLight, 20000, 'setTimeout called');
